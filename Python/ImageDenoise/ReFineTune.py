...
# Function to find Kaggle datasets
def find_kaggle_datasets():
    """Find and use COCO 2017 dataset in Kaggle input directory"""
    potential_coco_dirs = [
        INPUT_DIR / 'coco-2017-dataset' / 'coco2017' / 'val2017',
        INPUT_DIR / 'coco-2017-dataset' / 'coco2017' / 'train2017',
        INPUT_DIR / 'coco-2017-dataset' / 'coco2017' / 'test2017'
    ]
    
    found_datasets = []
    for dir_path in potential_coco_dirs:
        if dir_path.exists():
            found_datasets.append(dir_path)
            print(f"Found COCO 2017 dataset: {dir_path}")
    
    return found_datasets

# Updated function to get image files with optimized COCO 2017 dataset handling
def get_image_files(kaggle_datasets=None):
    """
    Get all image files from COCO 2017 dataset
    ...
    if not kaggle_datasets:
        print("No COCO 2017 dataset found in input directory")
        return []
    
    # Prioritize training and validation images
    image_files = []
    image_extensions = ['*.jpg', '*.jpeg', '*.png']
    
    for dataset_dir in kaggle_datasets:
        for ext in image_extensions:
            # Use list comprehension with Path.glob for efficiency
            dataset_images = list(dataset_dir.glob(ext))
            
            # Limit total number of images to prevent excessive memory usage
            max_images = 20000
            image_files.extend(dataset_images[:max_images - len(image_files)])
    ...
    print(f"Found {len(image_files)} images in COCO 2017 dataset")
...
def main():
    ...
    # Directly use Kaggle's COCO 2017 dataset
    kaggle_datasets = find_kaggle_datasets()
    
    if not kaggle_datasets:
        print("No COCO 2017 dataset found. Please check the dataset paths.")
        return
    ...
    train_patches, val_patches, test_patches = prepare_patches(
        image_files, 
        patch_size=64,  # Reduced from original
        stride=24,      # More aggressive patch extraction
        max_patches_per_image=15,  # Reduced to prevent overfitting
        max_total_patches=25000    # Reduced total patches
    )
    
    # Look for an existing fine-tuned model
    model_paths = list(Path(WORKING_DIR).glob('**/*.h5')) + list(Path(WORKING_DIR).glob('**/*.keras'))
    model_path = model_paths[0] if model_paths else None
    
    # Load the previously fine-tuned model
    if model_path:
        print(f"Loading previous fine-tuned model from {model_path}")
        with strategy.scope():
            model = load_model(model_path, compile=False)
            model.compile(
                optimizer=Adam(learning_rate=5e-5),  # Lower learning rate for re-fine-tuning
                loss=combined_loss,
                metrics=['mse']
            )
    else:
        # Create a new model if no previous model found
        model = build_improved_model(input_shape=(64, 64, 3))
    
    model.summary()
    
    # Create data generators with more aggressive regularization
    train_gen = DenoiseDataGenerator(
        train_patches, 
        batch_size=24,  # Reduced batch size
        noise_types=['gaussian', 'salt_pepper', 'speckle']  # More diverse noise
    )
    val_gen = DenoiseDataGenerator(
        ...
        noise_params={
            'gaussian_std': 0.12,      # Slightly higher noise level
            'salt_pepper_amount': 0.06 # More challenging noise
        }
    )
    
    # Setup callbacks with more aggressive early stopping and regularization
    callbacks = [
        ModelCheckpoint(
            str(MODEL_DIR / 'retuned_denoising_model_epoch_{epoch:02d}_val_loss_{val_loss:.4f}.keras'),
            monitor='val_loss',
            save_best_only=True,
            save_weights_only=False
        ),
        ModelCheckpoint(
            str(MODEL_DIR / 'retuned_denoising_model_best.keras'),
            monitor='val_loss',
            save_best_only=True,
            save_weights_only=False
        ),
        ...
        EarlyStopping(
            ...
            min_delta=0.001,  # More strict improvement requirement
            verbose=1
        )
    ]
    
    # Training configuration
    epochs = 80  # Reduced epochs to prevent overfitting
    ...
    # Visualize results
    save_path = WORKING_DIR / 're_finetuned_denoising_results_visualization.png'
    visualize_results(model, test_patches, save_path=save_path)
    
    # Save the final re-fine-tuned model
    final_model_keras_path = WORKING_DIR / 're_fine_tuned_denoising_model.keras'
    final_model_h5_path = WORKING_DIR / 're_fine_tuned_denoising_model.h5'
    
    model.save(final_model_keras_path)
    print(f"Re-fine-tuned model saved to {final_model_keras_path}")
    
    model.save(final_model_h5_path)
    print(f"Re-fine-tuned model saved to {final_model_h5_path}")
    
    # Optional: Save evaluation results
    import json
    results_path = WORKING_DIR / 're_model_evaluation_results.json'
