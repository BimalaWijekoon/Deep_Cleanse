...
def main():
    ...
    try:
        model_path = MODEL_DIR / "audio_denoiser_final.keras"
        if not model_path.exists():
            model_path = MODEL_DIR / "audio_denoiser_final.h5"
    ...
    for idx, audio_file in enumerate(audio_files):
        ...
        # Load audio
        original_audio, sample_rate = librosa.load(audio_file, sr=16000)
        ...
        for noise_name, noise_config in noise_types.items():
            ...
            # Prepare for model input (spectrogram conversion)
            def audio_to_spectrogram(audio):
                spec = np.abs(librosa.stft(audio, n_fft=2048, hop_length=512))
                spec = librosa.amplitude_to_db(spec, ref=np.max)
                spec = (spec - spec.min()) / (spec.max() - spec.min())
                return spec.T[:128, :128, np.newaxis]
            ...
            denoised_spec = model.predict(np.expand_dims(noisy_spec, axis=0))[0]
            ...
            def spectrogram_to_audio(spec):
                ...
                spec_db = librosa.db_to_amplitude(spec)
                return librosa.istft(spec_db.T, n_fft=2048, hop_length=512)
            
            denoised_audio = spectrogram_to_audio(denoised_spec[:, :, 0])
    ...
    with open(report_path, 'w') as f:
        f.write("AUDIO DENOISING MODEL VALIDATION REPORT\n")
        f.write("======================================\n\n")
        ...
        f.write("\nRECOMMENDATIONS:\n")
        f.write("---------------\n")
        f.write("1. Further noise type diversification in training data\n")
        f.write("2. Consider adaptive noise level estimation\n")
        f.write("3. Explore more sophisticated spectrogram transformation techniques\n")
